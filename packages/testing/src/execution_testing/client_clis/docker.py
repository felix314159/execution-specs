...
set -uo pipefail
port={port}
# The request is `stdin_file<TAB>binary<TAB>arg...`. This lightweight wrapper
# (used for consumers that never read stdin) sends an empty stdin_file field,
# so the server runs the command with no input redirection. The
# stdin-forwarding variant below is used for batch-capable consumers.
...
# Persistent-shell wrapper variant for batch-capable consumers (Besu,
# Nethermind), which feed many fixture-file paths to a single client process
# via stdin. The wrapper captures its stdin to a temp file on the shared temp
# mount and sends that file's path as the request's stdin_file field, so the
# server runs the containerized binary with its stdin redirected from that
# file (whose paths resolve in the container because the fixtures directory is
# mounted at its host path). A terminal stdin is skipped so an interactive run
# does not block; an empty/closed stdin yields an empty file, matching a local
# binary handed no input. Only generated for batch-capable consumers, so the
# per-file hot path of the other clients keeps its minimal fork count.
_WRAPPER_EXEC_STDIN_TEMPLATE = """\
#!/usr/bin/env bash
# Auto-generated by the EEST consume-direct Docker bridge.
# Dispatches `{binary}` into the session container's persistent shell,
# forwarding stdin.
set -uo pipefail
port={port}
stdin_file=$(mktemp "${{TMPDIR:-/tmp}}/eest-consume-direct-stdin-XXXXXX")
trap 'rm -f "$stdin_file"' EXIT
[[ -t 0 ]] || cat > "$stdin_file"
request="$stdin_file"$'\\t'{binary_quoted}
for arg in "$@"; do
  request+=$'\\t'"$arg"
done
exec 3<>"/dev/tcp/127.0.0.1/$port"
...
# `docker run` fallback variant for batch-capable consumers: forwards stdin
# (the batch's fixture-file paths) into the container with `-i`, and also
# bind-mounts the directory of every existing path *read from stdin* (not just
# the path arguments) so the containerized tool can open them. `set -e` is
# dropped so the tool's exit code can be captured and the temp file removed.
_WRAPPER_RUN_STDIN_TEMPLATE = """\
#!/usr/bin/env bash
# Auto-generated by the EEST consume-direct Docker bridge.
# Runs `{binary}` in an ephemeral container from `{image}`, forwarding stdin.
set -uo pipefail
image={image_quoted}
binary={binary_quoted}
declare -a forwarded=()
...
while IFS= read -r line; do
  [[ -n "$line" && -e "$line" ]] || continue
  mount_dir=$(dirname -- "$(realpath -- "$line")")
  if [[ -z "${{seen[$mount_dir]:-}}" ]]; then
    seen[$mount_dir]=1
    mounts+=("-v" "$mount_dir:$mount_dir")
  fi
...
docker run --rm -i --network=none {user_flag}"${{mounts[@]}}" \
  --entrypoint "$binary" "$image" \
  ${{forwarded[@]+"${{forwarded[@]}}"}} < "$stdin_file"
"""
...
class _CommandServer:
    ...
    def _run(self, tokens: Sequence[str]) -> Tuple[int, str, str]:
        ...
        ``tokens`` is ``[stdin_file, binary, *args]``; ``stdin_file`` (empty
        when the wrapper has no stdin to forward) is a host path on the shared
        temp mount that the command's stdin is redirected from. Arguments that
        name an existing host path are resolved to their absolute form (the
        container's working directory differs from the host's). The
        stdout/stderr files for this call are named here and their paths
        returned for the wrapper to read.
        """
        stdin_file, binary, *args = tokens
        resolved = [binary] + [
        ...
        redirect_in = (
            f"< {shlex.quote(stdin_file)} " if stdin_file else ""
        )
        line = (
            f"{command} {redirect_in}"
            f"> {shlex.quote(out_file)} 2> {shlex.quote(err_file)}; "
            f"printf '{marker}:%s\\n' \"$?\"\n"
        )
...
def _build_run_wrapper(
    ...
    *,
    ...
    template = (
        _WRAPPER_RUN_STDIN_TEMPLATE if forward_stdin else _WRAPPER_RUN_TEMPLATE
    )
    script = template.format(
        image=image,
        binary=binary,
...
def fixture_consumers_from_docker_image(
    ...
    *,
    ...
    for consumer_class, binary in client.consumers:
        # Batch-capable consumers (Besu, Nethermind) feed many fixture-file
        # paths to one client process via stdin; their wrappers must forward
        # stdin into the container. Other consumers never read stdin, so they
        # use the lighter wrappers that skip stdin capture on their hot path.
        forward_stdin = getattr(consumer_class, "batch_capable", False)
        if server is not None:
            wrapper = _build_exec_wrapper(
                server, binary, image, forward_stdin=forward_stdin
            )
