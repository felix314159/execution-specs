...
def test_besu_only_not_capped(
    command: ConsumeDirectCommand, monkeypatch: pytest.MonkeyPatch
) -> None:
    """
    A large Besu-only run uses `-n auto` like any other client.

    Besu used to be capped to a few workers because each fixture-file
    invocation cold-started a fresh JVM; now that Besu fixtures are batched
    into one long-lived process per worker, there is no special-casing.
    """
    ...
    assert args[-2:] == ["-n", "auto"]


def test_explicit_n_for_besu_is_untouched(
    command: ConsumeDirectCommand, monkeypatch: pytest.MonkeyPatch
) -> None:
    """An explicit `-n` for a Besu-only run is honored, not clamped."""
    monkeypatch.setattr("os.cpu_count", lambda: 16)
    ...
    assert execution.args.count("-n") == 1
    assert "16" in execution.args
